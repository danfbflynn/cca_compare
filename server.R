...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    ...
    co2_5 <- input$kwhslider * (rps_yr+5)/100 * input$co2avoid 
    co2_100 <- input$kwhslider * input$co2avoid
    
    plotdat <- c(bill5, -co2_5, bill100, -co2_100)
  ...
  output$co2avoid <- renderText({
    co2_5 <- input$kwhslider * (rps_yr+5)/100 * input$co2avoid # lbs of CO2 per kWh of renewable energy
    co2_100 <- input$kwhslider * input$co2avoid
    ...
    paste("This would be a <b>",
          round(r2, 1),
          "x</b> decrease in CO2 emissions compared to the +5% renewable option. </br>",
